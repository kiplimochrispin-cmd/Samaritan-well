...
if (productsRoot && cartLines && subtotalEl && deliveryEl && totalEl && statusEl && checkoutForm) {
  ...
  function validateCheckoutForm() {
    return validateBaseFields() && validatePaymentFields();
  }
  ...
  function renderProducts() {
    productsRoot.innerHTML = products
      .map(
        ...
        <article class="product-card">
          <h3>${product.name}</h3>
          <p class="price">${formatKes(product.price)} <span>${product.unit}</span></p>
          <div class="qty-row">
            <label for="${product.id}">Qty</label>
            <input id="${product.id}" type="number" min="0" value="0" data-product-id="${product.id}" />
          </div>
        </article>
      `,
      )
    ...
    productsRoot.querySelectorAll("input[data-product-id]").forEach((input) => {
      input.addEventListener("input", (event) => {
        const currentInput = event.currentTarget;
        const id = currentInput.getAttribute("data-product-id");
        const value = Math.max(0, Number(currentInput.value || 0));
  ...
  function getSubtotal() {
    return products.reduce((accumulator, product) => {
      return accumulator + product.price * quantities[product.id];
    }, 0);
  }
      ...
      cartLines.innerHTML = selected
        .map(
          ...
          <div class="cart-line">
            ...
            <strong>${formatKes(product.price * quantities[product.id])}</strong>
          </div>
        `,
        )
        .join("");
