...
const server = http.createServer(async (req, res) => {
    ...
    const reportPathBlocked = url.pathname === "/santa_shepherds_water_report.html" ||
    ...
    if (method === "POST" && url.pathname === "/api/orders") {
        try {
            ...
            insertOrderStmt.run(orderId, order.customerName, order.customerPhone, order.customerLocation, order.customerNotes, order.paymentMethod, order.paymentReference, Math.round(order.subtotal), Math.round(order.deliveryFee), Math.round(order.total), "pending", order.createdAt, JSON.stringify(order.items));
